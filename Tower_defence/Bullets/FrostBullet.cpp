...
FrostBullet::FrostBullet(sf::Vector2f startPos, std::shared_ptr<Enemy> targetEnemy, float spd, int dmg, float slowFactor, float areaEffectRadius, float slowDuration)
...
void FrostBullet::update(float deltaTime, const std::vector<std::shared_ptr<Enemy>>& enemies) {
